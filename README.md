...
> Each `Entity` wraps a tuple of `Detection`s (text, label, position, confidence). `to_dict()` exposes the raw values; `repr(detection)` deliberately masks the `text` field to prevent accidental leaks through logs.
